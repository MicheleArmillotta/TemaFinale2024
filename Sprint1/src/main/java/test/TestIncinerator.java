...
public class TestIncinerator {
	...
	public void test() {
		...
		IApplMessage start = CommUtils.buildRequest("tester", "start_test", "start_test(N)", "incineratorobserver");
		...
		try {
			...
			Thread.sleep(3000);
			IApplMessage activation_reply = connSupport.request(activation);
			CommUtils.outcyan("ACTIVATION: activation reply=" + activation_reply);
			String answer_act = activation_reply.msgContent();
			...
			assertEquals(answer_act, "activation_reply(ok)");
			assertEquals(answer_start, "start_reply(ok)");
