...
public class Testwis {
	...
	public void testStartRobot() {
		IApplMessage start = CommUtils.buildRequest("tester", "start_test", "act_test(N)", "wisobserver");
		try {
...
//inviamo messaggio di start all' observer (aspettiamo sia in idle)
			...
			CommUtils.outcyan("ACTIVATION: start reply=" + start_reply);
			String answer_act = start_reply.msgContent();
			CommUtils.outcyan("ACTIVATION PAYLOAD: "+ answer_act);
			
			assertEquals(answer_act, "start_reply(ok)");
	...
	public void testBurnin() {
		IApplMessage burn = CommUtils.buildRequest("tester", "burn_test", "burn_test(N)", "wisobserver");
		try {
			CommUtils.outmagenta("test_observer ======================================= ");
			while (connSupport == null) {
				connSupport = ConnectionFactory.createClientSupport(PROTOCOL, ADDRESS, PORT);
...
//inviamo messaggio di start all' observer (aspettiamo sia in idle)
			...
			Thread.sleep(3000);
			IApplMessage burn_reply = connSupport.request(burn);
			CommUtils.outcyan("ACTIVATION: start reply=" + burn_reply);
			String answer_act = burn_reply.msgContent();
			CommUtils.outcyan("ACTIVATION PAYLOAD: "+ answer_act);
			
			assertEquals(answer_act, "burn_reply(ok)");
